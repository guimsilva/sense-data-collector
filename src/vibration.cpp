...
void Vibration::printFFTData(double *vData, uint16_t bufferSize, uint8_t scaleType)
{
...
void Vibration::computeVibrationFFT(bool printResults)
{
    ...
    {
        Serial.println("Data:");
        printFFTData(vReal, samples, sclTime);
    }
    ...
    if (printResults)
    {
        ...
        printFFTData(vReal, samples, sclIndex);
        Serial.println("Computed Imaginary values:");
        printFFTData(vImag, samples, sclIndex);
    }
    FFT.complexToMagnitude(); /* Compute magnitudes */
    if (printResults)
    {
        Serial.println("Computed magnitudes:");
        printFFTData(vReal, (samples >> 1), sclFrequency);
    }
